...
- **Vuokratoiminta on erillinen näkymä `app/vuokra.html` (v1.15.0).** Henkilökohtainen näkymä (index.html) suodattaa vuokratilin rivit pois kaikista listoista, kuukausivalitsimesta ja laskureista `ptxs()`-funktiolla — mutta `computeBalances` käyttää yhä koko `txs`:ää, jotta vuokratilin saldo on oikein. Uusi näkymä: yleiskatsaus (kassavirta ennen/jälkeen lyhennysten, kk/vuosi), asunnot (per asunto + tyhjät kuukaudet + ylimääräiset vastikkeet), lainat (pankin "Jäljellä"-luku viestistä), tapahtumat (kohdistus asuntoon). API: `GET /transactions?account=Vuokra`, `PUT /transactions/:id {property_id}`.
- **`transactions.property_id` = Tuottokartan `kayttaja_asunnot.id`** (a1 Riistakatu 15 B 17, a2 Riistakatu 15 B 10, a3 Niiralankatu 19 B 37). Migraatio 008 (ajettu tuotantoon 24.9.2026 — ÄLÄ aja uudelleen, ALTER kaatuu duplicate columniin). Tuonnissa `rentalProperty()` kohdistaa: vastikeviite → lainan tilinumero → osoite viestissä → vuokralaisen nimi. **OP-tuonti tallentaa nyt viitteen `viesti`-kenttään**, koska se on ainoa tapa erottaa saman taloyhtiön kaksi vastiketta. Tuottokartan osoitteet (Riistakatu 15 A 17/A 10, Niiralankatu 17 A 28) ovat väärin, mutta niitä ei korjattu: Fund-trackerin `rental_snapshots` on avainnettu osoitteella.
...
- **Toistuva tilisiirto = sama arkistointitunnus joka kuukausi (OP).** Import antaa samalle id:lle eri päivällä/summalla päiväkohtaisen id:n (`<id>_<YYYY-MM-DD>`). Ennen korjausta vuokratililtä puuttui 6 × 100 € "Asuntosäästö"-riviä (2020–21). Vuokratili täsmää: 31.12.2025 2 521,70 €, 24.9.2026 2 041,91 €, alkusaldo 18.5.2020 3 247,67 €.
