...
- **Vuokratoiminta — vaiheet 1–3 ✅ v1.15.0 (24.9.2026):** henkilökohtainen näkymä ilman vuokrarivejä, `property_id` + automaattinen kohdistus, `vuokra.html` (yleiskatsaus / asunnot / lainat / tapahtumat). **Jatkossa:**
  - [ ] **Vaihe 4 — suunnitelma vs. toteuma:** read-only binding `TUOTTOKARTTA_DB` Oma-talouden workeriin (sama kuvio kuin Fund-trackerissa), asuntojen nimet/vuokra/vastike/laina Tuottokartasta → korvaa `vuokra.html`:n kovakoodatun `PROPS`-listan ja näyttää poikkeamat (vuokra ≠ suunniteltu, vastike muuttunut, lainasaldo vs. TK:n `oma_laina_saldo`).
  - [ ] **Vaihe 5 — vanhojen vuosien luokittelu:** 2020–2025 kulut sisältävät todennäköisesti henkilökohtaisia ostoja (Klarna, Stockmann, Amex, Indoor Group) ja kalustusta; `Vuokra — tulot` sisältää yksittäisiä omia siirtoja (esim. 16.9.2021 RUUSKANEN HENRI 1 003,45 €). EEPILA OY −59 000 € (10/2020) on asunnon osto — kohdistamatta. FI03-laina oletettu Niiralaksi (a3) ketjusta FI03→FI11→FI89; vahvista.
  - [ ] **Fund-tracker IT5-A16 taso 1:** lainasaldot Oma-talouden vuokratilin "Jäljellä"-havainnoista (`OMA_TALOUS_DB`-binding on jo olemassa).
  - [ ] Tuottokartan osoitteiden korjaus + `rental_snapshots`-avaimen migraatio id-pohjaiseksi (tehtävä yhdessä, ettei sarja katkea).
